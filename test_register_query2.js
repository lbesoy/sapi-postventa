...
// Try with full schema prefix as in original query
const cleanSQL = `SELECT T0."U_MARCA", T1."Name", T0."ItemCode", T0."ItemName", T2."ItmsGrpNam" FROM "OITM" T0 INNER JOIN "SBO_SAPI"."@OK_MARCA" T1 ON T0."U_MARCA" = T1."Code" INNER JOIN "OITB" T2 ON T0."ItmsGrpCod" = T2."ItmsGrpCod" ORDER BY T0."U_MARCA", T0."ItemName"`;
...
async function registerQuery() {
  ...
  if (json.success) {
    ...
    if (json2.data && json2.data.length > 0) {
      ...
      console.log("Error:", JSON.stringify(json2));
    }
  }
}
registerQuery();
