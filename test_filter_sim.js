...
async function test() {
    ...
    let page = 0;
    while(true) {
        const res = await axios.get(`${SUPABASE_URL}/rest/v1/refacciones?select=*`, {
            headers: { 'apikey': SUPABASE_KEY, 'Authorization': `Bearer ${SUPABASE_KEY}`, 'Range': `${page*1000}-${(page+1)*1000-1}` }
        });
        if (res.data && res.data.length > 0) {
            allRefacciones = allRefacciones.concat(res.data);
