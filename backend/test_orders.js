...
async function test() {
  try {
    ...
    const res = await sapApi.get(`${SAP_URL}/Orders?$filter=CardCode eq 'CL052' and DocumentStatus eq 'bost_Open'&$select=DocDate,DocNum,Comments,DocTotal`, {
      headers: { 'Cookie': `B1SESSION=${sessionId}` }
    });
    
    console.log("Ordenes abiertas encontradas:", res.data.value.length);
