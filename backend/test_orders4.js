...
async function test() {
  try {
    ...
    if(res.data.value.length > 0) {
      ...
      if (order.DocumentLines && order.DocumentLines.length > 0) {
        const line = order.DocumentLines[0];
        const keys = Object.keys(line).filter(k => k.toLowerCase().includes('open') || k.toLowerCase().includes('total') || k.toLowerCase().includes('sum') || k.toLowerCase().includes('bal'));
        keys.forEach(k => console.log("Line ->", k, ":", line[k]));
      }
    }
  } catch (e) {
