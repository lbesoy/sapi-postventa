export default async function handler(req, res) {
  ...
  if (authHeader.startsWith('Bearer ')) {
    ...
    if (supabaseUrl && supabaseAnonKey) {
      try {
        ...
        console.error('Auth verification error in sync-status:', err);
      }
    }
  }
  ...
  try {
    ...
    if (resp.ok) {
      ...
      if (latestRun) {
        return res.status(200).json({
          ...
          conclusion: latestRun.conclusion, // success, failure, etc.
    ...
    console.error('Error fetching GitHub Actions sync status:', error);
