...
import { getTemplateForStage, STAGES } from './utils/email-templates.js';
...
export default async function handler(req, res) {
  ...
  try {
    // 2. Determinar la Etapa de Destino y si hay cambio
    let stage = null;
    ...
    // Si no es una transición relevante de etapa, ignorar
    if (!stage) {
    ...
    // 4. Generar la Plantilla HTML y Asunto
    const { subject, html } = getTemplateForStage(stage, record, cliente);
    ...
    // 6. Configurar hilos (In-Reply-To / References)
    ...
    if (stage !== STAGES.REPORTADO && ultimoMsgId) {
    ...
    console.log(`[Notification Sent] Stage: ${stage}, Message-ID: ${info.messageId}`);
